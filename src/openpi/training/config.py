...
from openpi.policies import franka_policy, franka_pi0fast_policy
...
class MyFrankaDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[franka_policy.FrankaInputs(action_dim=32)],
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi0_franka_HW_pretrain",
        model=pi0.Pi0Config(),
        data=MyFrankaDataConfig(
            repo_id="hw/lerobot_generate_data",
            base_config=DataConfig(
                prompt_from_task=True,
            ),
        ),
    ...
    TrainConfig(
        name="pi0_franka_HW_finetune",  # *full* finetuning.
        model=pi0.Pi0Config(),
        data=MyFrankaDataConfig(
            repo_id="hw/lerobot_data_6_7_v1",
            base_config=DataConfig(
                prompt_from_task=True,
            ),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("/mnt/nodestor/dhy/code/openpi/checkpoints/pi0_franka_HW_v3_finetune/pick_red_cube_on_green_cube_10/5000/params"),
        num_train_steps=30_000,
        batch_size=56,
        fsdp_devices=7,
        wandb_enabled=False,
    ...
    TrainConfig(
        name="pi0fast_franka",# *full* finetuning.
        model=pi0_fast.Pi0FASTConfig(action_dim=7, action_horizon=10, max_token_len=180),
        data=SimpleDataConfig(
            repo_id="franka/train",
            assets=AssetsConfig(
                assets_dir="s3://openpi-assets/checkpoints/pi0_fast_base/assets",
                asset_id="droid",),
            data_transforms=lambda model: _transforms.Group(
                inputs=[franka_pi0fast_policy.FrankaPi0FastInputs(action_dim=model.action_dim,model_type=ModelType.PI0_FAST)],
                outputs=[franka_pi0fast_policy.FrankaPi0FastOutputs()],
            ),
            base_config=DataConfig(
                prompt_from_task=True,
            ),
    ...
    TrainConfig(
        ...
        data=SimpleDataConfig(
            ...
            assets=AssetsConfig(
                ...
                asset_id="droid",),
            data_transforms=lambda model: _transforms.Group(
                inputs=[franka_policy.FrankaInputs(action_dim=model.action_dim)],
                outputs=[franka_policy.FrankaOutputs()],
            ),
            base_config=DataConfig(
                prompt_from_task=True,
